...
public class OrderConsumerService : BackgroundService
{
    ...
    private async Task ProcessMessage(Message<string, string> message)
    {
        try
        {
            ...
            var orderMessage = JsonSerializer.Deserialize<List<OrderMessage>>(message.Value);
            _logger.LogInformation($"[CONSUMER {_consumerInstanceId}] Processing  {orderMessage?.Count} orders ");
            
            // Simulate processing time
            await Task.Delay(Random.Shared.Next(1000, 3000));
            
            orderMessage.ForEach(x =>
            {
                _logger.LogInformation($"[CONSUMER {_consumerInstanceId}] Successfully processed order: {x?.OrderId} with product: {x?.Product} in country {x.Country}");
            });
        }
